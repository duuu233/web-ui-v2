...
- 图库管理：公共图库图片的列表、详情、新增、编辑、物理删除和启禁用；列表可按图库分类筛选并将所选分类 ID 作为 `categoryId` 传给列表接口；删除按 `productImgId` 提交 `id`，后端同时清理 OSS 文件；单条图片同时维护简中、英语、繁中和日文标题与说明，列表不再按 `language` 筛选，并只使用 `imgThumb` 展示缩略图；新增/编辑原图时上传接口携带 `isUploadThumb=1`，并优先使用返回的 `urlThumb` 作为缩略图；新增、编辑和详情共用表单维护整数权重 `grade`，新增默认值及旧详情缺失字段时均回落为 `0`；适用产品/设备选择为非必填。
